...
int process()
{
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        {
            diagonal[i][j] += a[i][j];
            if(i>0&&j<m-1) diagonal[i][j] += diagonal[i-1][j+1];
        }
    }
    ...
    for(int i = 0; i < n; i++)
    {
        ...
        {
            dp[i][j] += vertical[i][j];
            if(i-k-1>=0) dp[i][j] -= vertical[i-k-1][j];
            if(j-1>=0)
            {
                dp[i][j] += dp[i][j-1];
                int d = j-k-1;
                if(d>=0) dp[i][j] -= diagonal[i][d];
                else if(i+d>=0) dp[i][j] -= diagonal[i+d][0];
                if(i-k-1>=0) dp[i][j] += diagonal[i-k-1][j];
            }
        }
    }

...
signed main()
{
    ...
    while(t--)
    {
        ...
        std::vector<int> p(n);
        std::vector<int> q(k);
        for(int i = 0LL; i < n; i++) std::cin >> p[i];
        for(int i = 0LL; i < k; i++) std::cin >> q[i];

        SegmentTree innerST(k);
        int innerInversions = 0LL;
        for(int i = 0LL; i < k; i++)
        {
            if(q[i]<k-1LL) innerInversions += innerST.query(q[i]+1LL, k-1LL);
            innerST.set(q[i], 1LL);
        }
        ...
        for(int i = 0LL; i < n; i++)
        {
            ...
            if(position[c]>0LL) outerInversions += inversionST.query(0LL, position[c]-1LL);
            outerInversions %= M;
            if(position[c]<n-1LL) 
            {
